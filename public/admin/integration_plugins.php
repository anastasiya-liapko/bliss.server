...
$next_order['url']='asc';
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Плагины".' </h2>
...
<th></th>
			...
			<th>
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=name&sort_order='. ($next_order['name']) .'\' class=\'sort\' column=\'name\' sort_order=\''.$sort_order['name'].'\'>name'. $sort_icon['name'].'</a>
			</th>
			...
			<th>
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=url&sort_order='. ($next_order['url']) .'\' class=\'sort\' column=\'url\' sort_order=\''.$sort_order['url'].'\'>Ссылка'. $sort_icon['url'].'</a>
			</th>
					<th></th>
				</tr>
		</thead><tbody>';
...
".(function_exists("processTD")?processTD("<td><span class='editable ' data-placeholder='' data-inp='text' data-url='engine/ajax.php?action=editable&table=integration_plugins' data-pk='{$item['id']}' data-name='url'>".htmlspecialchars($item['url'])."</span></td>", $item, "Ссылка"):"<td><span class='editable ' data-placeholder='' data-inp='text' data-url='engine/ajax.php?action=editable&table=integration_plugins' data-pk='{$item['id']}' data-name='url'>".htmlspecialchars($item['url'])."</span></td>")."
	...
	$actions['edit'] = function()
	{
		...
		if(isset($id))
		{
			$item = q("SELECT * FROM integration_plugins WHERE id=?",[$id]);
			$item = $item[0];
		}
				...
				<fieldset>'.
					...
					.'
						...
						<div class="form-group">
							...
							<div class="">
								<img src="'.$item["img_url"].'" style="max-width:200px; max-height:200px;" /><label for="img_url" class="file"> Выберите изображение <input type="file" name="img_url" id="img_url" /></label></div>
						</div>
	...
	$actions['edit_page'] = function()
	{
		...
		$html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Плагины".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
	...
	$actions['reorder'] = function()
	{
		...
		{
			qi("UPDATE `integration_plugins` SET `orderby` = ? WHERE id = ?", [$i, $line[$i]]);
		}
...
$url = $_REQUEST['url'];
		...
		$sql = "INSERT INTO integration_plugins (`name`, `img_url`, `url`) VALUES (?, ?, ?)";
		if(function_exists("processInsertQuery"))
		{
			$sql = processInsertQuery($sql);
		}

		qi($sql, [$name, $img_url, $url]);
		$last_id = qInsertId();
...
$set[] = is_null($_REQUEST['url'])?"`url`=NULL":"`url`='".addslashes($_REQUEST['url'])."'";
			...
			{
				$set = implode(", ", $set);
				$sql = "UPDATE integration_plugins SET $set WHERE id=?";
	...
	$actions['delete'] = function()
	{
		...
		try
		{
			qi("DELETE FROM integration_plugins WHERE id=?", [$id]);
	...
	function get_data($force_kill_pagination=false)
	{
		...
		if($pagination == 1)
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM integration_plugins main_table) temp $srch $filter $where $order LIMIT :start, :limit";
		...
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM integration_plugins main_table) temp $srch $filter $where $order";
			if(function_exists("processSelectQuery"))
			{
				$sql = processSelectQuery($sql);
			}
	...
	echo masterRender("Плагины", $content, 7);
